...
{
    public class KitchenEnqueueOrderUseCase : IKitchenEnqueueOrderUseCase
    {

        private readonly IKitchenQueueRepository _kitchenQueueRepository;
        private readonly IOrderRepository _orderRepository;

        public KitchenEnqueueOrderUseCase(
        ...
        public async Task ExecuteAsync(ManageKitchenRequest request)
        {
            ...
            if (kitchenQueue != null)
                throw new ApplicationException($"Order with ID {request.OrderId} is already in the kitchen queue.");

            kitchenQueue = new KitchenQueue(order);

            await _kitchenQueueRepository.BeginTransactionAsync();

            try
            {
                await _kitchenQueueRepository.AddAsync(kitchenQueue);
            }
            catch (Exception ex)
            {
                await _kitchenQueueRepository.RollbackAsync();
                throw new ApplicationException("An error occurred while enqueuing the order.", ex);
            }
